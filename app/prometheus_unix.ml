...
let pp f = function
  ...
  | Some (`Unix_domain_socket (`File path)) -> Fmt.pf f "unix:%s" path

let error fmt = Fmt.kstr (fun msg -> Error (`Msg msg)) fmt
...
let parse_tcp s host_port =
  ...
  | Some host, port, ("" | "/") when host <> "" ->
    ...
    if valid_port port then Ok (Some (`TCP (`Host host, `Port port)))
...
let of_string s =
  ...
  | Some port when valid_port port ->
    Ok (Some (`TCP (`Host default_host, `Port port)))
  ...
  | None ->
    ...
    | Some i ->
      ...
      | "unix" -> Ok (Some (`Unix_domain_socket (`File rest)))
...
let listen_prometheus =
  ...
  let addr = Arg.conv ~docv:"ADDR" (of_string, pp) in
  let doc =
